...
from lib.to_srt import STL, SRT
...
class Utils(object):
    ...
    @staticmethod
    def upload_files_to_s3(directory_path):
        try:
            ...
            bucket_name = "video-transcoder-srt-step3"
            
            # Path to directory
            path = Path(directory_path)
            
            # Dictionary to store filenames and their corresponding URLs
            uploaded_files = {}
            
            # Iterate over each .vtt file in the directory
            for file in path.glob('*.vtt'):
                file_name = file.name
                
                # Upload the file to S3
                s3.upload_file(str(file), bucket_name, file_name)
                
                # Generate the URL for the uploaded file (assuming the bucket is publicly accessible)
    ...
    def delete_file_from_s3(bucket, key):
        ...
        try:
            ...
            logging.info(f"File {key} deleted from bucket {bucket}")
        except Exception as e:
            logging.error(f"Error deleting file {key} from bucket {bucket}. Error: {e}")
    ...
    def extract_audio_from_video(input_video, output_audio):
        ...
        try:
            ...
            stream = stream.audio.output(output_audio, acodec='mp3', loglevel='error')
            ffmpeg.run(stream)
            logging.info(f"Audio extracted from {input_video} to {output_audio}")
        except Exception as e:
            logging.error(f"Error extracting audio from {input_video}. Error: {e}")

warnings.simplefilter('ignore')

from transcriptservices import TranscriptService
...
def remove_file(path):
    ...
    if os.path.exists(path):
        try:
            ...
            logging.error(f"Error removing file {path}. Error: {e}")
...
def remove_directory(path):
    ...
    if os.path.exists(path):
        try:
            ...
            logging.error(f"Error removing directory {path}. Error: {e}")

def process_message(s3_url, file_id, target_languages, model, connection, cursor):
    """
    Process the file from the provided S3 URL.
    """
    try:
        ...
        # Remove the 'https://' prefix and split the remainder into bucket and key
        url_parts = s3_url[8:].split('/', 1)

        # Ensure the URL was split into exactly two parts
        if len(url_parts) != 2:
            raise ValueError(f"URL does not contain both a bucket and a key: {s3_url}")

        bucket = url_parts[0].split('.')[0]
        key = url_parts[1]
        logging.info(f"Received file {key}")

        local_path = f'/home/ubuntu/final/video/{key}'
        output_audio_path = f'/home/ubuntu/final/audio/{Path(key).stem}.mp3'

        logging.info("Downloading file from S3...")
        start_time = time.time()
        Utils.download_file_from_s3(bucket=bucket, key=key, download_path=local_path)
        logging.info(f"Downloaded in {time.time() - start_time:.2f} seconds")

        logging.info("Extracting audio from video...")
        start_time = time.time()
        Utils.extract_audio_from_video(local_path, output_audio_path)
        logging.info(f"Extracted in {time.time() - start_time:.2f} seconds")
        ...
        start_time = time.time()
        transcript = model.transcribe(output_audio_path, verbose=False, language="en")
        logging.info(f"Transcribed in {time.time() - start_time:.2f} seconds")

        dir_path = Path(f"/home/ubuntu/final/transcripts/{Path(key).stem}")
        dir_path.mkdir(parents=True, exist_ok=True)
        # target_languages = ['spa', 'fra', 'deu', 'zho']
        ...
        vtt_writer = whisper.utils.get_writer("vtt", dir_path)
        output_vtt = f"{Path(key).stem}.vtt"
        vtt_writer(transcript, dir_path / output_vtt, options)

        ts.translate_vtt(dir_path, output_vtt, target_languages)

        Utils.upload_files_to_s3(dir_path)
        remove_file(output_audio_path)
        remove_directory(dir_path)

        logging.info(f"Successfully uploaded file {Path(key).stem} to s3 bucket.")

        # Mark the file as processed in the database only after all processing has completed successfully
        cursor.execute("UPDATE upload_s3_files SET VTT_gen = TRUE WHERE id = %s", (file_id,))
        connection.commit()

        # Utils.delete_file_from_s3(bucket, key)
        # logging.info(f"Successfully deleted file {key} from s3 bucket.")
    except Exception as e:
        logging.error(f"Error processing message {s3_url}. Error: {e}")
        cursor.execute("UPDATE upload_s3_files SET picked = FALSE WHERE id = %s", (file_id,))
        connection.commit()

# Rest of the main and other functions will remain largely unchanged, but with added logging and error handling.
